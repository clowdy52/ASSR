    ...
    rsg( rsg_ )
{
    ...
    try {
        ...
        //正面扑球
        readSkillsFromFile( "./skills/fight.skl" );
        //侧面扑球
        readSkillsFromFile( "./skills/fight2.skl" );//old

         readSkillsFromFile( "./skills/dajiao.skl" );

         readSkillsFromFile( "./skills/longkick.skl" );
         readSkillsFromFile( "./skills/kickclear.skl" );
         readSkillsFromFile( "./skills/stablekick1.skl" );
         readSkillsFromFile( "./skills/stablekick4.skl" );
	readSkillsFromFile( "./skills/middle_block.skl" );
	readSkillsFromFile( "./skills/left_block.skl" );
	readSkillsFromFile( "./skills/right_block.skl" );//new
...
bool NaoBehavior::noTeammateAround() {
    for(int i = WO_TEAMMATE1; i <= WO_TEAMMATE11; ++i) {
        ...
        if (Team->validPosition == true) {
            VecPosition TeamPosition = Team->pos;
            TeamPosition.setZ(0);
            double tempDis = TeamPosition.getDistanceTo(ball);
            if(tempDis > 3)
                continue;
            else
                return false;
...
bool NaoBehavior::noOppAround() {
	for(int i=WO_OPPONENT1;i<WO_OPPONENT1+NUM_AGENTS;++i) {
		WorldObject* Opp = worldModel->getWorldObject(i);
		if(Opp->validPosition == true) {
			VecPosition OppPosition = Opp->pos;
			OppPosition.setZ(0);
			double tempDis = OppPosition.getDistanceTo(VecPosition(-15,0,0));
            if(tempDis > 3)
                continue;
            else
                return false;
...
bool NaoBehavior::isOutside() {
	if(ball.getX() < -15 || ball.getX() > 15 || ball.getY() < -10 || ball.getY() > 10)
		return true;
	else
		return false;
}
