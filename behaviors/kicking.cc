...
//excalibur函数是比较机械但是稳定的踢球方式，虽然它的调整范围不能太大，但很稳定，适合用在开球和踢边线球的场合
...
//     }
   ...
   if (me.getDistanceTo(ball)>0.5){
      ...
      return SKILL_DAJIAO_LEFT_LEG;//此处返回动作
   }
...
SkillType NaoBehavior::kickBallAtTargetSimplePositioning(const VecPosition &targetToKickAt, SkillType kick_skill, int kickType) {
    ...
    //可在此定义大脚调整的各项参数
    if(kickType==3){
        stand_pos = ball + offX*0.85 ;//校内赛的同学可以调整优化此项参数得到更稳定的长踢效果
    }else if(kickType==4){
        stand_pos = ball + offX*0.78 -y_vec *0.12;
    }else if(kickType==6){
        //stand_pos = ball + offX*0.885 -y_vec *0.01;
        //stand_pos = ball + offX*0.885;
        //stand_pos = ball + offX*0.83 -y_vec *0.01;
        // stand_pos = ball + offX*0.885;
        stand_pos = ball + offX*0.869-y_vec*0.022;
    }else if(kickType==7){
        stand_pos = ball + offX -y_vec *0.03;
    }
    else{
...
SkillType NaoBehavior::LongKick(VecPosition target)
{
    ...
    if(num==1||num==2||num==5||num==6||num==7||num==8)//在此写入想要能踢大脚的机器人的编号，此大脚仅type0与type2机器人可使用，如需要可添加更多方式的长踢
    {
        return kickBall(3,target);
    }
