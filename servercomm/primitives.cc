...
string NaoBehavior::composeAction() {
    ...
    int closestNum= findClosestPlayer2Ball();
    string message;
    if (canTrust || fallen) {
        if(makeSayMessage(uNum, time, timeBallLastSeen, ballX, ballY, myX, myY, fallen, message,closestNum)) {
            ss << message;
            /*
            cout << "Data Sent:\n";
            cout << "\tTime: " << time << "\n";
